...
class FileChunker {
    ...
    loadOptionalDeps() {
        ...
        try {
            ...
            if (!jszip) {
                try { jszip = require(path.join(skillNodeModules, 'jszip')); } catch (e) { }
            }
            if (!xmldom) {
                try { xmldom = require(path.join(skillNodeModules, '@xmldom', 'xmldom')); } catch (e) { }
            }

            if (pdfParser && mammoth && XLSX && jszip && xmldom) return true;
            ...
            if (!jszip) { try { jszip = require('jszip'); } catch (e) { } }
            if (!xmldom) { try { xmldom = require('@xmldom/xmldom'); } catch (e) { } }
    ...
    async processFile(absolutePath) {
        ...
        const isDeepDoc = /\.(pdf|docx|xlsx|xls|pptx)$/i.test(ext);
